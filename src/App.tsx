...
import {
  ...
  QORTAL_GATEWAY_CANDIDATE_LIMIT,
...
export default function App() {
  ...
  const [qortalGroupDiscoveries, setQortalGroupDiscoveries] =
    useState<AsyncState<GroupData[]>>(createState([]));
  ...
  const [showGroupOnboarding, setShowGroupOnboarding] = useState(false);
  ...
  useEffect(() => {
    setShowGroupOnboarding(
      !!groupOnboardingAccountAddress &&
        !readGroupOnboardingDismissed(groupOnboardingNetwork, groupOnboardingAccountAddress),
    );
  }, [groupOnboardingAccountAddress, groupOnboardingNetwork]);

  function dismissGroupOnboarding() {
    if (groupOnboardingAccountAddress) {
      writeGroupOnboardingDismissed(groupOnboardingNetwork, groupOnboardingAccountAddress);
    }
    setShowGroupOnboarding(false);
  }
  ...
  const shouldDecryptSelectedGroupMessages =
    ...
    (selectedChat.network === 'qortal'
      ? shouldDecryptGroupMessages(selectedChat.group, {
          ...
          isAccountUnlocked: isQortalAccountUnlocked,
  ...
  const canUseQortalAccount = canUseQortalAccountForHost(
  ...
  const qortalGroupSendUnavailableLabel = !qortalAccount
    ? qortalAccountError || qortalAccountRequiredLabel
    : !isQortalAccountUnlocked
      ? accountLockedLabel
    : qortalBridge.value.isHomeBridge
  ...
  const selectedDirectHistoryUnavailable =
    selectedChat?.kind === 'direct' &&
    (selectedChat.network === 'qortal'
      ? !isQortalAccountUnlocked || !canReadQortalPrivateDirectChat
  ...
  const closedGroupHistoryUnavailableLabel = selectedChat?.network === 'qortal'
    ? !qortalAccount
      ? qortalAccountError || qortalAccountRequiredLabel
      : !isQortalAccountUnlocked
        ? accountLockedLabel
  ...
  // --- Qortal group search ---------------------------------------------------
  // Qortal Core has no group-name search endpoint. The first submitted query
  // loads the complete catalogue once for this page session; subsequent
  // searches are local and never require a group to have current chat activity.
  async function loadQortalGroupDiscoveries(
    ...
    try {
      let discoveries: GroupData[];

      if (!trimmedSearch) {
        discoveries = qortalGroups.value.filter((group) => group.groupId > 0);
      } else {
        const actionsKey = actionList.join('\n');
        let catalogue = qortalGroupCatalogueRef.current;

        if (!catalogue || catalogue.actionsKey !== actionsKey) {
          catalogue = {
            actionsKey,
            groups: await listGroups('qortal', actionList, 0),
          };
          qortalGroupCatalogueRef.current = catalogue;
        }

        discoveries = filterQortalGroupCatalogue(catalogue.groups, trimmedSearch);
      }
  ...
  async function loadQortalGatewayGroups(
    actionList = qortalBridge.value.actions,
    options: { quiet?: boolean } = {},
  ) {
    const requestId = ++qortalGatewayGroupRequestRef.current;

    if (!options.quiet) {
      setQortalGroups((current) => ({ phase: 'loading', value: current.value }));
    }

    try {
      const stats = await getQortalActiveGroupStats(QORTAL_GATEWAY_CANDIDATE_LIMIT);
      const discoveries = await discoverQortalGatewayGroups({
        loadGroup: (groupId) => getGroup('qortal', groupId, actionList),
        loadMessages: (group) =>
      ...
      if (qortalGatewayGroupRequestRef.current !== requestId) {
        return;
      }

      const selectedGroup =
        selectedChatRef.current?.kind === 'group' && selectedChatRef.current.network === 'qortal'
          ? selectedChatRef.current.group
          : null;
      const publicGroups = retainSelectedQortalGatewayGroup(
        discoveries.map(({ group }) => group),
        selectedGroup,
      );

      setQortalGroups({
        phase: 'ready',
        value: withGeneralChatGroup(publicGroups, '', t),
      });
      setQortalActiveChats({
        phase: 'ready',
        value: {
          direct: [],
      ...
      if (qortalGatewayGroupRequestRef.current !== requestId || options.quiet) {
        return;
      }
    ...
    try {
      ...
      if (
        ...
        void recoverMissingPrivateGroupKeys(
          ...
          chat.group,
          nextMessages,
          { address: qortalAccount.address, isUnlocked: isQortalAccountUnlocked },
          qortalActionList,
          { quiet: options.quiet },
        );
      }
  ...
  async function loadQortalDirectMessages(
    ...
    try {
      if (!isQortalAccountUnlocked || !hasAction(qortalActionList, 'SEARCH_PRIVATE_DIRECT_CHAT_MESSAGES')) {
        setMessagesChatKey(chatKey);
        setMessages({ phase: 'ready', value: emptyMessages });
        return;
      }
  ...
  async function handleQortalPublishGroupKey(group: GroupData) {
    ...
    try {
      ...
      void loadMessages({ group, kind: 'group', network: 'qortal' }, qortalBridge.value.actions, {
        accountUnlocked: isQortalAccountUnlocked,
        quiet: true,
        skipKeyRecovery: true,
      });
    } catch (error) {
  ...
  async function ensureQortalAccountWritable() {
    const selectedQortalAccount = qortalAccount;

    if (!selectedQortalAccount) {
      setWriteError(qortalAccountError || qortalAccountRequiredLabel);
      return null;
    }

    if (isQortalHub) {
      return {
        address: selectedQortalAccount.address,
        avatarUrl: null,
        isUnlocked: true,
        name: selectedQortalAccount.name,
      } satisfies QdnSelectedAccount;
    }
  ...
  function ensureWritableAccountForNetwork(network: ChatNetwork) {
    return network === 'qortal' ? ensureQortalAccountWritable() : ensureSelectedAccountUnlocked();
  }
  ...
  async function handleJoinQortalGroup() {
    ...
    try {
      const selectedAccount = await ensureQortalAccountWritable();
  ...
  async function runPendingSend(
    ...
    try {
      ...
      if (chat.kind === 'direct' && isCurrentWritablePendingTarget(entry.target, entry.accountAddress)) {
        if (network === 'qortal') {
          void loadQortalActiveChats(selectedAccount.address, actionList, { quiet: true });
        } else {
          void loadActiveChats(selectedAccount, actionList, { quiet: true });
        }
      }
      ...
      if (
        ...
        void loadMessages(chat, actionList, { accountUnlocked: selectedAccount.isUnlocked, quiet: true });
      }
    } catch (error) {
      if (!isCurrentOrRefreshingPendingOwner(entry.target, entry.accountAddress)) {
        return;
  ...
  function handleRetryPendingSend(localId: string) {
    ...
    void (async () => {
      const selectedAccount = await ensureWritableAccountForNetwork(entry.target.network ?? 'qortium');
  ...
  async function handleDeleteMessage(thread: MessageThread) {
    ...
    try {
      const selectedAccount = await ensureWritableAccountForNetwork(chat.network ?? 'qortium');
  ...
  async function initializeSession(accountRefreshCoordinator: StartupAccountRefreshCoordinator) {
    ...
    if (hasNetworkBridge('qortal') || hasLegacyQortalBridgeCandidate()) {
      ...
      const isAvailable = isQortalChatBridgeAvailable(nextQortalBridge);
      ...
      if (isAvailable && nextQortalBridge?.host === 'gateway') {
        ...
        setQortalAccountJoinRequests({ phase: 'ready', value: emptyJoinRequests });
        setQortalAdminJoinRequests({ phase: 'ready', value: emptyAdminJoinRequests });
        void loadQortalGatewayGroups(nextQortalBridge.actions, { quiet: true });
  ...
  useEffect(() => {
    ...
    if (!isQortalAccountUnlocked || !canReadQortalPrivateDirectChat || directs.length === 0) {
      return undefined;
    }
  ...
  // A public gateway has no selected account and therefore no account-active
  // chat feed. Refresh Core's bounded public groupstats seed while visible;
  // the loader keeps the currently selected open group for the session if its
  // last message expires between passes.
  useEffect(() => {
    if (!qortalPublicRuntime || !qortalAvailable || qortalBridge.phase !== 'ready') {
      return undefined;
    }
    ...
    const interval = window.setInterval(() => {
      ...
      void loadQortalGatewayGroups(qortalBridge.value.actions, { quiet: true });
    }, QORTAL_GATEWAY_GROUP_REFRESH_MS);

    return () => window.clearInterval(interval);
  }, [qortalAvailable, qortalBridge.phase, qortalBridge.value.actions.join('\n'), qortalPublicRuntime]);
            ...
            {!isGroupsCollapsed && showGroupOnboarding ? (
              <div className="panel__intro">
                ...
                <button
                  ...
                  onClick={dismissGroupOnboarding}
