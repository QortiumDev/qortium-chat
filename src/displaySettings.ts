...
export function getInitialDisplaySettings(): QdnDisplaySettings {
  ...
  return {
    ...
    uiStyle: normalizeUiStyle(query?.get('uiStyle') ?? hostWindow?._qdnUiStyle ?? hostWindow?._qdnUIStyle) ??
...
export function getDisplaySettingsUpdateFromMessage(
  ...
  switch (data.action) {
    ...
    case 'DISPLAY_SETTINGS_CHANGED': {
      ...
      const uiStyle = normalizeUiStyle(data.uiStyle ?? data.ui ?? data.qdnUiStyle) ?? current.uiStyle;
